...
public class Login extends JFrame
{
 ...
 public Login( )
 {
	 ...
	 loginButton.addActionListener(actionButton);
        ...
        this.userChoices = id.getText();
        this.userPass = new String(password.getPassword());
        
 }

 
 public static class Choices implements ActionListener         // What to do when button is pressed
 {
     Login login = new Login();
     
     public String username = login.userChoices;
     public String userPass = login.userPass;
	...
	// If login is pressed and username/password are NOT EMPTY
 	if(eventButton.getActionCommand().equals("Login"))
 	{
 		System.out.println(username);
 		  ...
 		  DBConnect connect = new DBConnect(username, userPass);
 		  try {
 			connect.userValidation(username, userPass);
 		} catch (Exception e) {
        ...
        boolean found = false;
	try {
		found = connect.userValidation(username, userPass);
	} catch (Exception e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	} // boolean value, defaulted to false, true whenever correct login information is entered
